{
  ...
  outputs = {
    ...
    overlay_invoke = nixpkgs: pythonPackages: let
      ifNotMinVersion = pkg: ver: overlay:
        if
          (
            nixlib.versionOlder pkg.version ver
          )
        then pkg.overrideAttrs overlay
        else pkg;
    in {
      huggingface-hub =
        ifNotMinVersion pythonPackages.huggingface-hub
        "0.13.2"
        (
          old: rec {
            version = "0.14.1";
      ...
      transformers =
        ifNotMinVersion pythonPackages.transformers
        "4.26"
        (
          old: rec {
            version = "4.28.1";
    ...
    overlay_pynixify = self: let
      ...
      mapCallPackage = pnames:
        builtins.listToAttrs (builtins.map (pname: {
            name = pname;
            value = callPackage (./packages + "/${pname}") {};
          })
          pnames);
      simplePackages = [
        "filterpy"
    ...
    overlay_amd = nixpkgs: pythonPackages: rec {
    ...
    packages.${system} = let
      ...
      invokeaiF = nixpkgs:
        nixpkgs.python3.pkgs.buildPythonApplication {
          ...
          propagatedBuildInputs = requirementsFor {
            pkgs = nixpkgs;
            nvidia = nixpkgs.nvidia;
          };
